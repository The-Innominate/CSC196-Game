...
void SpaceGame::Update(float dt){
	...
	case SpaceGame::eState::Game:
		...
		if (m_spawnTimer >= m_spawnTime) {
			m_spawnTimer = 0;
			std::unique_ptr<Enemy> enemy = std::make_unique<Enemy>(kda::randomf(75.0f, 150.0f), kda::pi, kda::Transform((kda::random(800), kda::random(600)), kda::random((kda::pi2), 1)), kda::g_modelManager.get("Enemy.txt"));
			enemy->m_tag = "Enemy";
			enemy->m_game = this;
